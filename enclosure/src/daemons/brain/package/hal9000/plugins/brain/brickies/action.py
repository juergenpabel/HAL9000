			...
			if parameter_value.find(':') >= 2:
				...
				if parser == 'jsonpath':
					...
					if len(matches) == 1:
						...
						logging.getLogger(str(self)).warning("jsonpath selector '{}' yielded multiple results, using first one".format(selector))
						parameter_value = matches[0].value
					else:
						logging.getLogger(str(self)).warning("jsonpath selector '{}' yielded no result".format(selector))
						parameter_value = ""
