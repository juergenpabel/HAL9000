		...
		if cortex['#activity']['audio'].module == "none":
			if 'volume' in signal:
				if self.alsamixer is None:
					self.daemon.arduino_show_gui_overlay('error', {"text": "NO SOUND DEVICE"}, 10)
					return
				if 'delta' in signal['volume']:
					if cortex['enclosure']['volume']['mute'] is False:
						...
						self.daemon.arduino_show_gui_overlay('volume', ({"level": str(cortex['enclosure']['volume']['level']), "mute": "false"}), 3)
				if 'mute' in signal['volume']:
					if signal['volume']['mute'] == "on":
						cortex['enclosure']['volume']['mute'] = True
						self.set_alsa_volume(0)
						self.daemon.arduino_show_gui_overlay('volume', ({"level": str(cortex['enclosure']['volume']['level']), "mute": "true"}), 0)
