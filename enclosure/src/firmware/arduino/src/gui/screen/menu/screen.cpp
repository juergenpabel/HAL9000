...
void gui_screen_menu(bool refresh) {
	...
	if(menu_title != g_system_runtime["gui/screen:menu/title"] || menu_item != g_system_runtime["gui/screen:menu/text"]) {
		...
		g_gui.setTextSize(1);
		g_gui.fillRect(0, (TFT_HEIGHT/8*3)-(g_gui.fontHeight()/2), TFT_WIDTH, g_gui.fontHeight(), TFT_BLACK);
		g_gui.drawString(menu_title.c_str(), TFT_WIDTH/2, TFT_HEIGHT/8*3);
		g_gui.setTextSize(2);
		g_gui.fillRect(0, (TFT_HEIGHT/8*4)-(g_gui.fontHeight()/2), TFT_WIDTH, g_gui.fontHeight(), TFT_BLACK);
		g_gui.drawString(menu_item.c_str(), TFT_WIDTH/2, TFT_HEIGHT/8*4);
	}
}
