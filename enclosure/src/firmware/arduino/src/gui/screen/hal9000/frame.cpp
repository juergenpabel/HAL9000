...
static int hal9000_frame(JPEGDRAW *pDraw) {
	for(int y=0; y<pDraw->iHeight; y++) {
		memcpy(&g_gui_buffer[(pDraw->y+y)*TFT_WIDTH+pDraw->x], &pDraw->pPixels[y*pDraw->iWidth+0], pDraw->iWidth*sizeof(uint16_t));
	}
	return true;
}
...
void gui_screen_hal9000_frame_draw(uint8_t* jpeg_data, uint32_t jpeg_size) {
	...
	for(int y=0; y<TFT_HEIGHT/2; y++) {
		for(int x=0; x<TFT_WIDTH/2; x++) {
			g_gui_buffer[(y*TFT_WIDTH)+(TFT_WIDTH-x-1)] = g_gui_buffer[(y*TFT_WIDTH)+(x)];
		}
		memcpy(&g_gui_buffer[(TFT_HEIGHT-y-1)*(TFT_WIDTH)], &g_gui_buffer[(y)*(TFT_WIDTH)], TFT_WIDTH*sizeof(uint16_t));
	}
	for(int y=0; y<TFT_HEIGHT; y++) {
		for(int x=0; x<TFT_WIDTH; x++) {
			if(g_gui_overlay.readPixel(x,y) == TFT_WHITE) {
				g_gui_buffer[(y*TFT_WIDTH)+x] = TFT_WHITE;
			}
		}
	}
