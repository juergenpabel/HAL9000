...
bool Settings::load() {
	...
	if(!file) {
		g_util_webserial.send("syslog", LogString("Settings::load('").append(this->filename).append(LogString("') => failed to open file")));
		return false;
	}
	if(file.size() == 0) {
		file.close();
	...
	do {
		...
		if(line_end_pos == line.npos) {
			file.close();
			this->clear();
			g_util_webserial.send("syslog", LogString("Settings::load('").append(this->filename).append(LogString("') => no newline")));
			return false;
		}
