...
class Daemon(HAL9000):
	...
	def configure(self, configuration: ConfigParser) -> None:
		...
		self.mqtt.subscribe("hal9000/daemon/arduino-service/#")
		...
		for peripheral_name in configuration.getlist(str(self), 'peripherals'):
			module_device = configuration.getstring(peripheral_name, 'device', fallback='hal9000.daemon.arduino.device.arduino')
			...
			module_driver = configuration.getstring(peripheral_name, 'driver', fallback='hal9000.daemon.arduino.driver.webserial')
		...
		if 'hal9000.daemon.arduino.driver.webserial' in self.Drivers:
			Driver = self.Drivers['hal9000.daemon.arduino.driver.webserial']
	...
	def on_event(self, device_type: str, device_name: str, event: str, value: str=None) -> None:
		...
		if self.mqtt is not None:
			mqtt_topic = 'hal9000/daemon/arduino-service/event:{}/{}'.format(device_type, device_name)
	...
	def on_command(self, client, userdata, message) -> None:
		...
		if self.webserial is None:
			...
			return
		if message.topic.startswith("hal9000/daemon/arduino-service/command:"):
