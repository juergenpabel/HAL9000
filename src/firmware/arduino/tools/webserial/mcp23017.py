...
def handler(self, line: str):
	if line is not None:
		if line.startswith('["application:runtime"'):
			data = json.loads(line)[1]
			if "status" in data and data["status"] == "configuring":
		...
		if line.startswith('["device:event"'):
			...
			if data["device"]["name"] == "volume":
				if data["device"]["type"] == "rotary":
					...
					hal9000.send('["gui:overlay", {"volume": {"level": "%s", "mute": "false"}}]' % self.volume)
				if data["device"]["type"] == "toggle":
					hal9000.send('["gui:overlay", {"none": {}}]')
